// gcc gst-launch.c `pkg-config --cflags --libs gstreamer-1.0 ` -DGST_DISABLE_OPTION_PARSING -DGST_PACKAGE_ORIGIN=\"Sailfish\" -DGST_API_VERSION=\"1.0\"
...
static void on_acquired(audioresource_t *audio_resource, bool acquired, void *user_data)
{
    ...
    gst_element_get_state(pipeline, &state, &pending, GST_MSECOND);
...
#ifdef SAILFISH
    resource = audioresource_init(AUDIO_RESOURCE_MEDIA, on_acquired, &pipeline);
    if (!resource) {
      g_printerr (_("ERROR: cannot acquire audio resources.\n"));
      goto end;
    }
